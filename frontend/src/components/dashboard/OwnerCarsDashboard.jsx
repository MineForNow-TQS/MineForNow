...
import { Car, Plus, Edit, Trash2, MapPin, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function OwnerCarsDashboard() {
    ...
    // Fetch cars owned by this user (using email as unique identifier)
    const { data: cars = [], isLoading } = useQuery(
        ['ownerCars', user?.email],
        () => carService.getCarsByOwner(user?.email).then(res => res.data)
    );

    // Mutation to toggle car active status
    const toggleActiveMutation = useMutation(
        ({ carId, isActive }) => carService.updateCar(carId, { status: isActive ? 'available' : 'unavailable' }),
        {
            onSuccess: () => {
                queryClient.invalidateQueries(['ownerCars', user?.email]);
            }
        }
    );
    ...
    const handleToggleActive = (carId, currentStatus) => {
        const isActive = currentStatus === 'available';
        toggleActiveMutation.mutate({ carId, isActive: !isActive });
    };
        ...
        <>
            ...
            {cars.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {cars.map((car) => {
                        const isActive = car.status === 'available';
                        return (
                            <Card key={car.id} className="overflow-hidden border border-slate-200">
                                ...
                                <div className="p-4">
                                    ...
                                    <div className="flex items-center gap-2 mb-4 pb-4 border-b border-slate-200">
                                        <button
                                            onClick={() => handleToggleActive(car.id, car.status)}
