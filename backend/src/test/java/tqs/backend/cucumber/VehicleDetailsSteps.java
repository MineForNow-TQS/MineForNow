...
public class VehicleDetailsSteps {
    ...
    @E("o veículo com ID {int} é um {string} de {string}")
    ...
    @E("o proprietário do veículo é {string}")
    public void oProprietarioDoVeiculoE(String nomeProprietario) {
        // Este passo é apenas documentação
        assertNotNull(nomeProprietario);
    }

    @Dado("que estou na página de detalhes do veículo com ID {int}")
    public void queEstouNaPaginaDeDetalhesDoVeiculoComID(int id) {
        ...
        page.getByRole(AriaRole.LINK, new Page.GetByRoleOptions().setName("Pesquisar").setExact(true)).click();
        page.waitForLoadState(LoadState.NETWORKIDLE);
    ...
    @Então("devo ver o nome do veículo {string}")
    ...
    @E("devo ver o tipo de combustível {string}")
    ...
    @E("devo ver a transmissão {string}")
    ...
    @E("devo ver o número de lugares {string}")
    ...
    @E("devo ver o número de portas {string}")
    ...
    @Então("devo ver a característica {string}")
    ...
    @Então("devo ver a cidade {string}")
    ...
    @E("a descrição deve conter {string}")
    ...
    @Então("o preço deve estar no formato correto com {string}")
